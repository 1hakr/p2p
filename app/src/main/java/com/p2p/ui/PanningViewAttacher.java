...
public class PanningViewAttacher implements ViewTreeObserver.OnGlobalLayoutListener {
	...
	public static final int DEFAULT_PANNING_DURATION_IN_MS = 30000;
	public static final int DEFAULT_ALT_PANNING_DURATION_IN_MS = 10000;
	...
	private void animate_() {
		...
		if(mWay == null) {
			...
			mDuration = mIsPortrait ? DEFAULT_PANNING_DURATION_IN_MS : DEFAULT_ALT_PANNING_DURATION_IN_MS;
		}
	...
	private void animate(float start, float end, long duration) {
		...
		mCurrentAnimator.setDuration(Math.abs(duration));
